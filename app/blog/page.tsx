...
export default function Blog() {
  ...
  return (
    <div className="container mx-auto px-4 py-8">
      <h1 className="text-3xl font-semibold mb-8 text-center">Blog Artikel</h1>
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {articles.map((article) => (
          <CardArticle key={article.id} title={article.title} excerpt={article.excerpt} slug={article.slug} imageUrl={article.imageUrl} date={article.date} category={article.category} />
        ))}
      </div>
    </div>
  );
